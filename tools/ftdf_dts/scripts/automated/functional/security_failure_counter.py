...
while( idx < len( msgFlow ) ):
    ...
    if( res == False ):
        ...
        if( msgFlow[idx+3] == 1 ):
            securityFailureCount = ret['PIBAttributeValue']['securityFailureCount']
        if( msgFlow[idx+3] == 2 and 
            (securityFailureCount+1) != ret['PIBAttributeValue']['securityFailureCount'] ):
                raise StopScript( 'SCRIPT: ERROR: securityFailureCount did not count the security failure' )
                result = False
                break
            ...
            if( res2 == False ):
                ...
                result = False
                break
            elif ret2['msgId'] != ftdf.FTDF_COMM_STATUS_INDICATION:
                logstr = ( 'SCRIPT: ERROR: Expected FTDF_COMM_STATUS_INDICATION, instead received ', ret2['msgId'] )
                raise StopScript( ''.join( map( str, logstr ) ) )
                result = False
                break
