...
msgFlow = ( devId1, msgRESET, ftdf.FTDF_RESET_CONFIRM,
      ...
      devId2, msgSET_SecurityDisabled, ftdf.FTDF_SET_CONFIRM,
...
while( idx < len( msgFlow ) ):
  ...
  if( res == False ):
    ...
    if( ret['msgId'] == ftdf.FTDF_DATA_CONFIRM ):
      ...
      # Check comm status frame received as expected
      ...
      if( res2 == False ):
        ...
        logstr = ( 'SCRIPT: ERROR: Expected COMM_STATUS_INDICATION confirm, instead received ', ret2['msgId'] )
        raise StopScript( ''.join( map( str, logstr ) ) )
        result = False
        break
      else:
        if( ret2['status'] != ftdf.FTDF_UNSUPPORTED_SECURITY ):
          raise StopScript( 'SCRIPT: ERROR: expected UNSUPPORTED_SECURITY' )
          result = False
          break
