...
##################################
# Set macMetricsEnabled
##################################
msg = { 'msgId': ftdf.FTDF_SET_REQUEST,
...
##################################
# Get macPerformanceMetrics
##################################
msg = { 'msgId': ftdf.FTDF_GET_REQUEST,
...
# Save current RX counters
rxCurrentCnt = ret['PIBAttributeValue']['RXSuccessCount']

# sleep 3 seconds then retrieve the counters again
time.sleep(3)
...
rxDeltaCnt = ret['PIBAttributeValue']['RXSuccessCount'] - rxCurrentCnt

if rxDeltaCnt < 11 or rxDeltaCnt > 13:
    raise StopScript( "Expected +/-12 keep alive frames after 3 seconds, instead rcvd: " + str(rxDeltaCnt) )


##################################
# Send keep alive request with period = 0 (disable)
##################################
...
if rxDeltaCnt != 0:
    raise StopScript( "Expected 0 keep alive frames after 3 seconds, instead rcvd: " + str(rxDeltaCnt) )
