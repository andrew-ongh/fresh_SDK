...
## - 1< Reset Arbiter  
## - 1< Set Arbiter configuration 0=(BLE, 4), 1=(FTDF, 2) and ignore BLE
## - 1< Enable FTDF Rx
## - 1< Start BLE active scanning
## - 1< Stop BLE active scanning
## - 1< Disable FTDF Rx
## - 1< Get arbiter stats and verify them: 
##   txpass1 == 0, rxpass1 == 0, txmask1 == 0, rxmask1 == 0, 
##   txpass2 == 0, rxpass2 == 1, txmask2 == 0, rxmask2 == 0
## - 1< Reset Arbiter 
...
## - 1< Reset Arbiter 
## - 1< Set Arbiter configuration 0=(FTDF, 2), 1=(BLE, 4) and ignore FTDF
## - 1< Enable FTDF Rx
## - 1< Start BLE active scanning
## - 1< Stop BLE active scanning
## - 1< Disable FTDF Rx
...
# Set arbiter configuration so that BLE active scanning is above FTDF Rx but ignore BLE
DTS_ArbiterSetConfig(devId1, 0x07f2, 0, 0, 
                     ftdf.DTS_COEX_MAC_TYPE_BLE, 4,
                     ftdf.DTS_COEX_MAC_TYPE_FTDF, 2,
                     ftdf.DTS_COEX_MAC_TYPE_EXT, 0,
                     ftdf.DTS_COEX_MAC_TYPE_NONE, 0,
...
# BLE TxPassed should be 0
if ret['txrxMonTxPassed1'] != 0:
    logstr = ( 'SCRIPT: ERROR: Expected zero txrxMonTxPassed1, instead received ', 
               ret['txrxMonTxPassed1'] )
    raise StopScript( ''.join( map( str, logstr ) ) )

# BLE RxPassed should be zero
if ret['txrxMonRxPassed1'] != 0:
    logstr = ( 'SCRIPT: ERROR: Expected zero txrxMonRxPassed1, instead received ', 
               ret['txrxMonRxPassed1'] )
    raise StopScript( ''.join( map( str, logstr ) ) )  
...
# BLE Rx Masked should be 0
if ret['txrxMonRxMasked2'] != 0:
    logstr = ( 'SCRIPT: ERROR: Expected 0 txrxMonRxMasked2, instead received ', 
               ret['txrxMonRxMasked2'] )
    raise StopScript( ''.join( map( str, logstr ) ) )

# BLE Tx Passed should be 1
if ret['txrxMonRxPassed2'] != 1:
    logstr = ( 'SCRIPT: ERROR: Expected 1 txrxMonRxPassed2, instead received ', 
               ret['txrxMonRxPassed2'] )
    raise StopScript( ''.join( map( str, logstr ) ) )
...
# Set arbiter configuration so that FTDF Rx is above BLE active scanning
DTS_ArbiterSetConfig(devId1, 0x07f0, 0, 0, 
                     ftdf.DTS_COEX_MAC_TYPE_FTDF, 2,
                     ftdf.DTS_COEX_MAC_TYPE_BLE, 4,
                     ftdf.DTS_COEX_MAC_TYPE_EXT, 0,
...
if ret['txrxMonTxPassed1'] != 0:
    logstr = ( 'SCRIPT: ERROR: Expected 0 txrxMonTxPassed1, instead received ', 
               ret['txrxMonTxPassed1'] )
    raise StopScript( ''.join( map( str, logstr ) ) ) 
...
# Set arbiter configuration so that  FTDF Rx is above BLE active scanning but ignore FTDF
DTS_ArbiterSetConfig(devId1, 0x07f4, 0, 0, 
                     ftdf.DTS_COEX_MAC_TYPE_FTDF, 2,
                     ftdf.DTS_COEX_MAC_TYPE_BLE, 4,
                     ftdf.DTS_COEX_MAC_TYPE_EXT, 0,
                     ftdf.DTS_COEX_MAC_TYPE_NONE, 0,
...
if ret['txrxMonRxPassed2'] == 0:
    logstr = ( 'SCRIPT: ERROR: Expected non-zero txrxMonRxPassed2, instead received ', 
               ret['txrxMonRxPassed2'] )
    raise StopScript( ''.join( map( str, logstr ) ) )
