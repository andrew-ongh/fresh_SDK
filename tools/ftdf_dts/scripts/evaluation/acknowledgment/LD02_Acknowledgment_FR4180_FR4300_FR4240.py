...
if(result):
    ...
    if( res == False ):
        ...
        res, ret = DTS_getMsg( devId1, responseTimeout )
        if( res == True ):
            raise StopScript( 'SCRIPT: ERROR: Found PHY is in RX_ON' )
            result = False
...
if(result):
    ...
    if( res == False ):
        ...
        logstr = ( 'SCRIPT: ERROR: Expected RX_ENABLE_CONFIRM confirm, instead received ', msgNameStr[ ret['msgId'] -1 ])
        raise StopScript( ''.join( map( str, logstr ) ) )
        result = False
    elif( ret['status'] != ftdf.FTDF_SUCCESS ):
        raise StopScript( 'SCRIPT: ERROR: PHY to RX_ON failed' )
        result = False
    ...
    if( res == False ):
        ...
        if( res == False ):
            ...
            if( ret['frame'][2] != dns ):
                ...
                raise StopScript( ''.join( map( str, logstr ) ) )
                result = False
            elif( ret['frame'][1] != 0x0 ):
                raise StopScript( 'SCRIPT: ERROR: received frame no standard Acknowledgment frame' )
                result = False
            else:
                # Check received frame
                expectedFrame = [2, 0, 172, 222, 218]
