...
# TEST: Enhanced Acknowledgment decription and verification
# Data frame
...
msgFlow = (
    ...
    devId2, msgSET_frameCounterMode_4, ftdf.FTDF_SET_CONFIRM,
...
while( idx < len( msgFlow ) ):
    ...
    if( res == False ):
        ...
        if msgFlow[idx+2] == ftdf.FTDF_DATA_CONFIRM:
            # Empty que from send data message
...
# TEST: Enhanced Acknowledgment frame Auxiliary Header
#------------------------------- Data Frame:
# Frame Control:
...
if(result):
    ...
    if( res == False ):
        ...
        if( res == False ):
            ...
            expectedFrame = [10, 32, 1, 28, 241, 255, 255, 255, 8, 9, 10, 11, 12, 13, 14, 15, 5, 131, 182]
...
# TEST: Enhanced Acknowledgment frame DNS must match frame to be Acknowleged when using no security 
#------------------------------- Data Frame:
dns = 0xAB
...
if(result):
    # Disable security on Receiving DUT
