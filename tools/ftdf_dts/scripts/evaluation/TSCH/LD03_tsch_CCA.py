...
msg = { 'msgId': ftdf.FTDF_SET_REQUEST, 'PIBAttribute': ftdf.FTDF_PIB_TRANSACTION_PERSISTENCE_TIME, 'PIBAttributeValue': 0x4 }
DTS_sndMsg( 1, msg )
DTS_sndMsg( 2, msg )
...
msg = { 'msgId': ftdf.FTDF_SET_REQUEST, 'PIBAttribute': ftdf.FTDF_PIB_MAX_FRAME_RETRIES, 'PIBAttributeValue': 7 }
...
DTS_setRegister(devId1,0x50002B04, 2, 0x40)
...
DTS_setRegister(1,0x50002B04, 2, 0xE1)
...
if (res1 == False):
    ...
    raise StopScript( "Incorrect response received from device: ret1=%s" % (ret1['status']))
    result = False
...
if (res1 == False):
    raise StopScript( "Response received from device!" )
    result = False

DTS_setRegister(1,0x50002B04, 2, 0x00)
...
msg = { 'msgId': ftdf.FTDF_SET_REQUEST, 'PIBAttribute': ftdf.FTDF_PIB_MAX_FRAME_RETRIES, 'PIBAttributeValue': 0xff }
...
if (res2 == False):
    raise StopScript( "Unexpected response received from device!" )    
    result = False
