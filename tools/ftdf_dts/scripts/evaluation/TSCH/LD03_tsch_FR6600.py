...
# This script was used manually to check the Enh Ack frame on the interface.
# These where Enh Ack frame requested in a dataframe, multipurpose frame, command frame

# Select the frame of intreset in the msgFlow
...
msgFlow = (
    ...
    devId1, msg_Link_Add_Dev1_1, ftdf.FTDF_SET_LINK_CONFIRM,
    devId2, msg_Link_Add_Dev2_1, ftdf.FTDF_SET_LINK_CONFIRM,
    devId1, msg_Link_Add_Dev1_2, ftdf.FTDF_SET_LINK_CONFIRM,
    devId2, msg_Link_Add_Dev2_2, ftdf.FTDF_SET_LINK_CONFIRM,
    ...
    # Generate data frame Enh Ack
    #devId1, msgDATA_Ack, ftdf.FTDF_DATA_CONFIRM,

    # Generate data frame Multipurpose Enh Ack
    #devId1, msgDATA_MultiPurpose_Ack, ftdf.FTDF_DATA_CONFIRM,

    # Generate data frame Enh Ack
    devId1, msgSET_ASSOC_PERMIT, ftdf.FTDF_SET_CONFIRM,
    #devId1, msgDisassociate_Request, ftdf.FTDF_DISASSOCIATE_CONFIRM
    devId2, msgASSOCIATE_Request, ftdf.FTDF_ASSOCIATE_CONFIRM
)
...
while( idx < len( msgFlow ) ):
    ...
    if( msgFlow[idx+2] == ftdf.FTDF_SCAN_CONFIRM ):
        idx += 3
        continue
    elif( msgFlow[idx+2] == ftdf.FTDF_ASSOCIATE_CONFIRM ):
        ...
        if( res == False ):
            ...
            DTS_sndMsg( devId1, msgASSOCIATE_Response )
            res, ret = DTS_getMsg( devId1, responseTimeout )
            if( res == False ):
                raise StopScript( 'SCRIPT: ERROR: No response received from device' )
                result = False
