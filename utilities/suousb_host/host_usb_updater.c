...
bool do_firmware_update(unsigned char *imagebuf, uint32_t size, uint32_t suousbbuffsz)
{
         ...
         */
        unsigned char *suousb_gpio_map_prodk = "SUOUSB_GPIO_MAP 0 4 00050102\n"; // ProDK
        ...
        error = issue_command_get_ok(suousb_gpio_map_prodk, buff, &len);
        if (error) {
        ...
        if (size - xfered) {
                ...
                error = wait_for_specific_response_or_abort("INFO SUOUSB_CMP_OK", 3000, buff, &len);
        }
