...
SUOUSB_LOADER_PATH="$sdkroot/projects/dk_apps/features/suousb_loader"
...
if [ x"$PRODUCT_ID" == x"DA14680-01" -o x"$PRODUCT_ID" == x"DA14681-01" ] ; then
    SUOUSB_BUILD_CONFIG="DA14681-01-Release_QSPI"
else
    SUOUSB_BUILD_CONFIG="DA14683-00-Release_QSPI"
fi
...
if [ -z "$boot_loader_bin" ] ; then
  boot_loader_bin="$SUOUSB_LOADER_PATH/$SUOUSB_BUILD_CONFIG/suousb_loader.bin"
fi
...
"$sdkroot/utilities/scripts/suousb/v11/mkimage.sh" "$bin_file" "$image_file" || exit 2
...
if [ -z $nobootloader ] ; then
  ...
  pushd "$QSCR"
  ./program_qspi_jtag.sh --cfg "$CFG" "$boot_loader_bin"|| exit 2
  popd
fi
